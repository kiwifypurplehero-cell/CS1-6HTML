...
(function exposeDesertArena(root, factory) {
    ...
    const SPAWNS = {
        ...
        dm: [[-43, 8], [-18, -5], [0, -30], [0, 31], [18, 22], [47, 22], [38, -36], [-34, -38]],
    };
    ...
    const api = Object.freeze({ name: MAP_NAME, create, validateLayout });

    // Contrato padronizado consumido pelo carregador do jogo. O mapa continua
    // autocontido neste arquivo; o HTML conhece apenas o catálogo e esta API.
    if (typeof window !== "undefined") {
        window.GAME_MAPS = window.GAME_MAPS || {};
        window.GAME_MAPS.de_dust2 = {
            id: "de_dust2",
            name: "de_dust2",
            build(context) {
                if (!context || !context.THREE || !context.scene) {
                    throw new TypeError("de_dust2 requer THREE e scene no contexto de construção.");
                }
                ...
                if (Array.isArray(context.wallObjects)) {
                    arena.group.traverse((object) => {
                        if (object.isMesh && !object.name.startsWith("bombsite-") && object.name !== "ground") {
                            context.wallObjects.push(object);
                        }
                    });
                }
                return arena;
